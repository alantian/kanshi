...
class Decoder(chainer.Chain):
    def __init__(self, charset_size, hidden_size, n_layers):
        ...
        with self.init_scope():
            ...
            self.gru = L.NStepGRU(n_layers=n_layers, in_size=hidden_size, out_size=hidden_size, dropout=0.1)
    ...
    def sample(self, batch_size=1, use_random=True, temperature=1.0, max_len=40):
        ...
        with chainer.no_backprop_mode(), chainer.using_config('train', False):
            ...
            for i in range(max_len):
                ...
                if use_random:
                    ...
                    ys = [
                        np.random.choice(len(row), p=normalize_p(np.power(row, temperature)))
                        for row in swy
                    ]
